...
if __name__ == '__main__':
    ...
    f.write("python "+BASIC_PATH+"EPSIII/gns_solver.py --train=1 --mode=prod --path="+BASIC_PATH+"EPSIII/ \n")
    f.write("deactivate\n")
    f.close()
    ...
    for size in SIZES:
        for instance in range(START_IDX, END_IDX+1):
            ...
            f.write("pip install --upgrade pip --no-index\n")
            f.write("pip install --no-index -r "+BASIC_PATH+"requirements.txt\n")
            f.write("python "+BASIC_PATH+"EPSIII/gns_solver.py --train=0 --mode=prod --size="+size+" --id="+str(instance)+" --path="+BASIC_PATH+"EPSIII/ \n")
            f.write("desactivate\n")
            f.close()
