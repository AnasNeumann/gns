...
PROBLEM_SIZES = [['s'], ['s', 'm', 'l', 'xl', 'xxl']] # ['s', 'm', 'l', 'xl', 'xxl', 'xxxl']
PPO_CONF = {
    "validation_rate": 20,
    "switch_batch": 10,
...
def train_or_validate_batch(reward_MEMORIES: Memories, agents: list[(Module, str)], batch: list[Instance], agent_names: list[str], train: bool, epochs: int, optimizer: Optimizer, solve_function: Callable, device: str, trainable: list, fixed_alpha: float = -1):
    """
...
def scheduling_stage(train_data: list[Instance], val_data: list[Instance], agents: list[(Module, str)], embedding_stack: Module, shared_critic: Module, solve_function: Callable, path: str, epochs: int, iterations:int, batch_size: int, switch_batch: int, validation_rate: int, device: str, interactive: bool=False, debug_mode: bool=False):
    ...
    for iteration in range(iterations):
        ...
        train_or_validate_batch(reward_MEMORIES=reward_MEMORIES, agents=agents, batch=current_batch, train=True, epochs=epochs, optimizer=scheduling_optimizer, agent_names=[AGENTS[SCHEDULING]], solve_function=solve_function, device=device, trainable=[0,1,0], fixed_alpha=1.0)
        if iteration % validation_rate == 0:
            debug_print("\t time to validate the loss...")
            agents[SCHEDULING][AGENT].eval()
            with torch.no_grad():
                current_vloss: MAPPO_Loss = train_or_validate_batch(reward_MEMORIES=reward_MEMORIES, agents=agents, batch=val_data, train=False, epochs=-1, optimizer=None, solve_function=solve_function, agent_names=[AGENTS[SCHEDULING]], device=device, trainable=[0,1,0], fixed_alpha=1.0)
...
def material_use_stage(train_data: list[Instance], val_data: list[Instance], agents: list[(Module, str)], embedding_stack: Module, shared_critic: Module, solve_function: Callable, path: str, epochs: int, iterations:int, batch_size: int, switch_batch: int, validation_rate: int, device: str, interactive: bool=False, debug_mode: bool=False):
    ...
    for iteration in range(iterations):
        ...
        if iteration % switch_batch == 0:
            debug_print(f"\t time to sample new batch of size {batch_size}...")
            current_batch: list[Instance] = random.sample(train_data, batch_size)
        train_or_validate_batch(reward_MEMORIES=reward_MEMORIES, agents=agents, batch=current_batch, train=True, epochs=epochs, optimizer=material_optimizer, solve_function=solve_function, agent_names=[AGENTS[MATERIAL_USE]], device=device, trainable=[0,0,1], fixed_alpha=1.0)
        if iteration % validation_rate == 0:
            debug_print("\t time to validate the loss...")
            agents[MATERIAL_USE][AGENT].eval()
            with torch.no_grad():
                current_vloss: MAPPO_Loss = train_or_validate_batch(reward_MEMORIES=reward_MEMORIES, agents=agents, batch=val_data, train=False, epochs=-1, optimizer=None, agent_names=[AGENTS[MATERIAL_USE]], solve_function=solve_function, device=device, trainable=[0,0,1], fixed_alpha=1.0)
                vlosses.add(current_vloss)
                #_vloss_TRACKER.update(current_vloss.value_loss)
...
def outsourcing_stage(train_data: list[Instance], val_data: list[Instance], agents: list[(Module, str)], embedding_stack: Module, shared_critic: Module, solve_function: Callable, path: str, epochs: int, iterations:int, batch_size: int, substage: int, switch_batch: int, validation_rate: int, device: str, interactive: bool=False, debug_mode: bool=False):
    ...
    for iteration in range(iterations):
        ...
        if iteration % switch_batch == 0:
            debug_print(f"\t time to sample new batch of size {batch_size}...")
            current_batch: list[Instance] = random.sample(train_data, batch_size)
        train_or_validate_batch(reward_MEMORIES=reward_MEMORIES, agents=agents, batch=current_batch, train=True, epochs=epochs, optimizer=outsourcing_optimizer, agent_names=[AGENTS[OUTSOURCING]], solve_function=solve_function, device=device, trainable=[1,0,0])
        if iteration % validation_rate == 0:
            debug_print("\t time to validate the loss...")
            agents[OUTSOURCING][AGENT].eval()
            with torch.no_grad():
                current_vloss: MAPPO_Loss = train_or_validate_batch(reward_MEMORIES=reward_MEMORIES, agents=agents, batch=val_data, train=False, epochs=-1, agent_names=[AGENTS[OUTSOURCING]], optimizer=None, solve_function=solve_function, device=device, trainable=[1,0,0])
                vlosses.add(current_vloss)
                #_vloss_TRACKER.update(current_vloss.value_loss)
...
def multi_agent_stage(train_data: list[Instance], val_data: list[Instance], agents: list[(Module, str)], embedding_stack: Module, shared_critic: Module, solve_function: Callable, path: str, epochs: int, iterations:int, batch_size: int, switch_batch: int, validation_rate: int, device: str, interactive: bool=False, debug_mode: bool=False):
    ...
    for iteration in range(iterations):
        ...
        if iteration % switch_batch == 0:
            debug_print(f"\t time to sample new batch of size {batch_size}...")
            current_batch: list[Instance] = random.sample(train_data, batch_size)
        train_or_validate_batch(reward_MEMORIES=reward_MEMORIES, agents=agents, batch=current_batch, train=True, epochs=epochs, optimizer=multi_agent_optimizer, agent_names=[name for _,name in agents], solve_function=solve_function, device=device, trainable=[1,1,1])
        if iteration % validation_rate == 0:
            debug_print("\t time to validate the loss...")
            for agent,_ in agents:
                agent.eval()
            with torch.no_grad():
                current_vloss: MAPPO_Loss = train_or_validate_batch(reward_MEMORIES=reward_MEMORIES, agents=agents, batch=val_data, train=False, epochs=-1, agent_names=[name for _,name in agents], optimizer=None, solve_function=solve_function, device=device, trainable=[1,1,1])
                vlosses.add(current_vloss)
                #_vloss_TRACKER.update(current_vloss.value_loss)
                #_material_loss_TRACKER.update(current_vloss.get(AGENTS[MATERIAL_USE]).policy_loss)
                #_scheduling_loss_TRACKER.update(current_vloss.get(AGENTS[SCHEDULING]).policy_loss)
